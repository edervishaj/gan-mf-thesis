...
class fullGANMF(BaseRecommender):
    RECOMMENDER_NAME = 'fullGANMF'
    ...
    def build(self, num_factors=10, emb_dim=32):
        ...
        def generator(condition):
            with tf.variable_scope('generator', reuse=tf.AUTO_REUSE):
                user_embeddings = tf.layers.dense(condition, units=num_factors, kernel_initializer=glorot_uniform, name='user_embeddings')
            ...
            fake_data = tf.matmul(user_embeddings, item_embeddings, transpose_b=True)
            return fake_data
    ...
    def fit(self, num_factors=10, emb_dim=32, epochs=300, batch_size=32, d_lr=1e-4, g_lr=1e-4, d_steps=1, g_steps=1,
        ...
        self.user_id = tf.placeholder(dtype=tf.float32, shape=[None, self.num_items])
        ...
        while not self._stop_training and epoch < epochs + 1:
            ...
            for _ in range(d_steps):
                ...
                while start_idx < len(all_users):
                    ...
                    _, _dloss = self.sess.run([dtrain, dloss], {real_profile: real_histories, self.user_id: real_histories})
            ...
            for _ in range(g_steps):
                ...
                while start_idx < len(all_users):
                    ...
                    uids = all_users[start_idx: end_idx]
                    real_histories = self.URM_train[uids].toarray()
                    _, _gloss = self.sess.run([gtrain, gloss], {real_profile: real_histories, self.user_id: real_histories})
    ...
    def _compute_item_score(self, user_id_array, items_to_compute=None):
        ...
        if self.mode == 'item':
            ...
            while start_idx < self.num_users:
                ...
                predictions = self.sess.run(self.fake_profile, {self.user_id: tmp_URM[start_idx: end_idx].toarray()})
            ...
            while start_idx < len(user_id_array):
                ...
                predictions = self.sess.run(self.fake_profile, {
                    self.user_id: self.URM_train[user_id_array[start_idx: end_idx]].toarray()})
                out = np.vstack((out, predictions))
                start_idx = end_idx
            return out