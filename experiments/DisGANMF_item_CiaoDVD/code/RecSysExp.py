        ...
        if len(params) > 0:
            ...
            if checkpoint_exists:
                ...
                if self.method == 'bayesian':
                    results = gp_minimize(self.obj_func, params, n_calls=evals, x0=previous_run.x_iters,
                                          y0=previous_run.func_vals, random_state=seed, verbose=True,
                                          callback=[checkpoint_saver])
                else:
                    results = dummy_minimize(self.obj_func, params, n_calls=evals, x0=previous_run.x_iters,
                                          y0=previous_run.func_vals, random_state=seed, verbose=True,
                                          callback=[checkpoint_saver])
            else:
                if self.method == 'bayesian':
...
def main(arguments):
    ...
    list_experiments = ['TopPop', 'Random', 'PureSVD', 'ALS', 'NMF', 'BPR', 'IRGAN', 'CFGAN', 'GANMF', 'DisGANMF']
